function trace = clusterSegments(trace, x, ud)
    ...
    [~, index] = sort(seglength,'descend');
    sorted_index = segIndex(index,:);

    mix_label= searchTracesRecursive(x',ud',sorted_index, sigma, winlen);
    labels = indexArrange(mix_label, index);
...
function labels = indexArrange(cluster_idx, index)
...
function indx = searchTracesRecursive(x, ud, chpoints, sigma, winlen)
    % global sigma winlen;
    ...
    if size(x,1) ~= 0
        ...
        num_segments = size(chpoints,1); % number of segments in data
        if num_segments ==1
            indx = [1;-1];
            return;
        end
