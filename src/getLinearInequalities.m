function [trace,label_guard] = getLinearInequalities(trace, iter, threshDist, inNum, num_var, Time)
% Find linear inequalities for the hybrid automata
    ...
    [state_trans, ~, ic ]= unique(trans(:,1:2), 'stable', 'rows');
    num = 1;
    for i = 1:size(state_trans,1)
        ...
        [wb_temp, inlayer_temp] = estimateLinearInequalities(points, num_vars,iter,threshDist,inNum);
        if isempty(wb_temp)
            continue;
        end
...
% Get segment given trace data and the corresponding index
