...
global act_function nL 
    ...
    str_node = x;
    for kji=1:nL+1
        str_node = extractdata(node_params{2*kji-1})*str_node+extractdata(node_params{2*kji});
        if strcmpi(act_function, "linear")
            continue
        elseif strcmpi(act_function,"tanh")
            str_node = tanh(str_node);
        elseif strcmpi(act_function, "sigmoid")
            str_node = 1./(1+exp(-str_node));
        else
            error("Wrong layer type")
        end
    end
    W2 = extractdata(node_params{end-1});
    b2 = extractdata(node_params{end});
    str_node = vpa(W2*str_node+b2,4);
    if strcmpi(act_function,"tanh")
