function getNonlinearHyst(file_name,label_guard, num_var, ode, pta_trace, PolyDegree)
...
for i = 1 : length(ode)
    modesToFlows{i} = eqs2string(ode(i), varNames, PolyDegree);
end
...
for iv = 1:length(varNames)
    ...
    ode0str = strcat(ode0str, [string(varNames(iv)), ' == 0']);
end
...
% add variables to ha
for i_var = 1:numVar
    ha.variables.add(java.lang.String(string(varNames(i_var))));
end
...
for i = 1 : length(varNames)
    initialExpression = strcat(initialExpression, ' & ', string(varNames(i)), ' == 0'); % todo: pick a reasonable initial condition
end
...
function MonCoeff = symCoeffs(varNames,PolyDegree)
...
function eqs = MonomialEquations(PolyDegree, varNames, Weights)
    MonCoeff = symCoeffs(varNames, PolyDegree);
    Dimension = length(varNames);
    all_coeffs = kron(eye(Dimension),MonCoeff); % One set of monomials for each dimension
    eqs_list = Weights{1}'*all_coeffs; % Creates a list of equations in order
...
function out = eqs2string(Weights,varNames,PolyDegree)
    eqs = MonomialEquations(PolyDegree, varNames, Weights);
    odeStrConjunction = '';
    for eq = 1:length(varNames)
        odeStr = strcat(string(varNames(eq)), ' == ', eqs{eq});
        if eq > 1
            odeStrConjunction = strcat(odeStrConjunction, " & ");
        end
        odeStrConjunction = strcat(odeStrConjunction, ' ', odeStr);
%         eqs{eq} = java.lang.String(odeStr);
    end
    out = odeStrConjunction;
end
